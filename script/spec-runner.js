...
let runnerToRun = null
if (args.runners) {
  runnerToRun = args.runners
  if (!['main', 'remote'].includes(runnerToRun)) {
    console.log(`${fail} runners must specify either 'main' or 'remote'`)
    process.exit(1)
  }
  console.log('Only running:', runnerToRun)
} else {
  console.log('Triggering both main and remote spec runners')
}
...
async function runElectronTests () {
  ...
  for (const [runnerId, { description, run }] of runners) {
    if (runnerToRun && runnerToRun !== runnerId) {
